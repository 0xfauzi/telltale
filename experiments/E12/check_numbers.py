"""Every number in docs/experiments/E12.md, against the JSON under experiments/E12/out/.

E08's checker, pointed at this experiment. It reads the numbers back out of the prose
and asks, of each, whether any number in `out/` matches it at the precision it was
written to. A number that matches nothing is printed with its line, and the write-up
says why: some are protocol constants quoted from protocol.md, some are dates, and some
are E05's cost figures cited as E05's. Nothing here decides which is which.

    uv run python experiments/E12/check_numbers.py
"""
...
E12_DIR = Path(__file__).resolve().parent
OUT = E12_DIR / "out"
DOC = E12_DIR.parents[1] / "docs" / "experiments" / "E12.md"
