"""`telltale experiment probe`. Spec 14.3.
...
kind is added to the `experiment` subparser cli.py already builds, so
`telltale experiment --help` lists them all in one place.
"""
...
from telltale import config, experiments, experiments_measure
...
def add_kinds(kinds: argparse._SubParsersAction[Any]) -> None:
    """Register the kind on the `experiment` subparser cli.py builds."""
