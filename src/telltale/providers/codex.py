...
`CAPABILITIES` and `DRIFT` are measurement tables and not parser code, so they live in
codex_drift.py and are re-exported here: providers/__init__.py's Provider protocol asks
a provider MODULE for both names, and claude.py splits the same way for the same reason.
...
from telltale.providers.codex_drift import CAPABILITIES as CAPABILITIES
from telltale.providers.codex_drift import DRIFT as DRIFT
from telltale.providers.codex_drift import SURFACES as SURFACES
